...
public class Coming implements Initializable {
    @FXML
    private TableView<Sell> tableComing;
    @FXML
    ...
    public void pressBtnLoad() {
        try {
            ...
            ResultSet rs = conn.createStatement().executeQuery("select * from store.storage");
            while (rs.next()) {
        ...
        C3.setCellValueFactory(new PropertyValueFactory<Sell, Integer>("count"));
        C4.setCellValueFactory(new PropertyValueFactory<Sell, Integer>("cost"));
        ...
        tableComing.setItems(null);
        tableComing.setItems(data);
    }
}
